"""
Evaluation script: Quality & Diversity metrics for generated images.

Two complementary metric pairs are computed:

  QUALITY  metrics  (are generated images realistic / on the real manifold?)
  -------------------------------------------------------------------------
  Precision  [Kynkäänniemi et al., 2019]  — fraction of generated images that
             fall inside the real data manifold (k-NN ball around each real image).
             Range [0, 1], higher = better quality.

  Density    [Naeem et al., 2020]         — like Precision but counts how *deeply*
             inside the real manifold each generated image is, not just whether it
             is inside.  More robust to outliers.
             Range [0, ∞), values > 1 are possible; higher = better quality.

  DIVERSITY metrics  (do generated images cover all real modes?)
  -------------------------------------------------------------------------
  Recall     [Kynkäänniemi et al., 2019]  — fraction of real images that are
             covered by at least one generated image's k-NN ball.
             Range [0, 1], higher = better diversity / mode coverage.

  Coverage   [Naeem et al., 2020]         — fraction of real images whose k-NN
             ball contains at least one generated image.  Fixes Recall's tendency
             to be fooled by a single near-duplicate covering many real images.
             Range [0, 1], higher = better diversity / mode coverage.

Usage
-----
python eval_quality_diversity.py \\
    --encoding_path path/to/encodings.npz \\
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc=desc):
            if isinstance(batch, (list, tuple)):
                batch = batch[0]
            # Key "2048" = pool3 features after AvgPool. torch_fidelity does NOT squeeze
            # the spatial dims for "2048" (unlike "64"/"192"/"768"), so we do it here
            # to get the required (B, 2048) shape for torch.cdist.
            feats = extractor(batch.to(device))[0].squeeze(-1).squeeze(-1)  # tuple[0] = "2048" (pool3)
            all_features.append(feats.cpu().float())
            # Free GPU memory immediately — we only need the CPU features
